...
// Timer : will hold the time between two clicks in milliseconds
...
$(document).keypress(function(event) {
	...
	if (callibrating){
		if(event.key == " "){
			caliClicks[calibration_i] = Date.now()
			console.log(caliClicks)
			calibration_i++
			console.log(calibration_i)
			if(calibration_i>1){
				callibrating = false
				calibration_i = 0
				DCL_SPEED = caliClicks[1] - caliClicks[0]
				$("#callibrating").text("Click 'c' to calibrate your speed")
				$("#callibrating").css('backgroundColor', 'white');
				$("#speed").text(DCL_SPEED)
			}
		}else{
			$("#callibrating").text("double click the spacebar with the required speed")
			$("#callibrating").css('backgroundColor', 'red');
		}
	}else{
		if (event.key == "c"){
			callibrating = true
			$("#callibrating").text("double click the spacebar with the required speed")
			$("#callibrating").css('backgroundColor', 'yellow');
		}
		if (event.key == " ") {
			switchKeys()
		}
...
// The function that switchs/press the key
